...
// Phase 2 compatible data structures
...
var llmCache = &LLMCache{
	...
	maxAge:  time.Hour * 24, // Cache for 24 hours
}
...
func main() {
	...
	fmt.Println("\n🎉 Phase 1 Beta USP Optimization completed successfully! 🎉")
}
...
func loadCacheFromDisk() {
	cacheFile := "../output/llm_cache.json"
	data, err := os.ReadFile(cacheFile)
	if err != nil {
		return // Cache file doesn't exist, start fresh
	}
	
	var diskCache map[string]CacheEntry
	if err := json.Unmarshal(data, &diskCache); err != nil {
		return // Invalid cache file, start fresh
	}
	
	llmCache.mutex.Lock()
	llmCache.entries = diskCache
	llmCache.mutex.Unlock()
	
	// Clean expired entries
	llmCache.cleanup()
	
	fmt.Printf("📦 Loaded %d cached LLM responses\n", len(diskCache))
}

// Save cache to disk
func saveCacheToDisk() {
	llmCache.mutex.RLock()
	defer llmCache.mutex.RUnlock()
	
	cacheFile := "../output/llm_cache.json"
	data, err := json.Marshal(llmCache.entries)
	if err != nil {
		return
	}
...
func callOllamaBatch(requests []BatchRequest) []BatchResponse {
	...
	for i, req := range requests {
		...
		go func(idx int, request BatchRequest) {
			...
			response, err := callOllama(request.Model, request.Prompt)
...
// Optimized callOllama with caching
func callOllama(model, prompt string) (string, error) {
	// Generate cache key
	cacheKey := generateCacheKey(model, prompt)
	
	// Check cache first
	if cachedResponse, found := llmCache.get(cacheKey); found {
		fmt.Printf("   🎯 Cache hit (%s)... ", model)
		fmt.Printf("✅ Response from cache (%d chars)\n", len(cachedResponse))
	...
	// Cache the response
	llmCache.set(cacheKey, ollamaResp.Response, model)
...
func processUSPOptimization(selectedModel, originalConcept string, selectionRubric CriteriaRubric, authorPersona AuthorPersona) (Phase1Output, error) {
	...
	// Compile results with Phase 2 compatibility
	result := Phase1Output{
		Timestamp:              time.Now().Format(time.RFC3339),
		...
		Recommendations: []ContentRecommendation{
			{
				Title:          finalUSP.USP,
				Confidence:     finalUSP.OverallScore * 100,
				Rank:           1,
				MarketGapScore: calculateMarketGapScore(marketIntel),
				ContentType:    "book",
			},
		},
		TotalGenerated:   1,
...
// Calculate market gap score for Phase 2 compatibility
func calculateMarketGapScore(intel MarketIntelligence) float64 {
	// Convert market saturation to gap score (inverse relationship)
	// Lower saturation = higher gap score
	gapScore := (1.0 - intel.MarketSaturationLevel) * 10.0
	
	// Adjust based on market gaps count
	if len(intel.MarketGaps) > 3 {
		gapScore += 1.0
	}
...
func displayCacheStats() {
	llmCache.mutex.RLock()
	totalEntries := len(llmCache.entries)
	llmCache.mutex.RUnlock()
	
	fmt.Printf("\n📊 LLM CACHE STATISTICS\n")
	fmt.Printf("══════════════════════════════════════════════════════════════════════\n")
	fmt.Printf("📦 Total cached responses: %d\n", totalEntries)
	fmt.Printf("⏱️  Cache duration: %v\n", llmCache.maxAge)
	fmt.Printf("💾 Cache file: ../output/llm_cache.json\n")
	fmt.Printf("🎯 Cache benefits: Faster responses, reduced API calls, cost savings\n")
}
...
// Score all concepts against all criteria with optimized batching
func scoreAllConcepts(concepts []ConceptSuggestion, rubric CriteriaRubric, selectedModel string) ([]ConceptEvaluation, error) {
	...
	fmt.Printf("\n🎯 Scoring %d concepts against %d criteria (batch optimization)...\n", len(concepts), len(rubric.Criteria))
	
	// Create batch requests for all concept-criteria combinations
	...
	for i, concept := range concepts {
		for j, criteria := range rubric.Criteria {
			requestID := fmt.Sprintf("concept_%d_criteria_%d", i, j)
...
Respond with ONLY a number between 0.0 and 1.0:`, 
			...
			requestMap[requestID] = struct {
				ConceptIndex  int
				CriteriaIndex int
			}{ConceptIndex: i, CriteriaIndex: j}
		}
	}
	
	fmt.Printf("📦 Processing %d scoring requests in parallel batches...\n", len(batchRequests))
	
	// Process batch requests
	batchResponses := callOllamaBatch(batchRequests)
	
	// Initialize evaluations
	for _, concept := range concepts {
		evaluations = append(evaluations, ConceptEvaluation{
			Concept: concept,
			Scores:  make([]ConceptScore, len(rubric.Criteria)),
		})
	}
	
	// Process batch responses
	for _, response := range batchResponses {
		indices, exists := requestMap[response.ID]
		if !exists {
			continue
		}
		...
		if response.Error == nil {
			scoreStr := extractNumericScore(response.Response)
			parsedScore := parseFloat(scoreStr)
			if parsedScore > 0 {
				score = parsedScore
			}
		}
		
		criteria := rubric.Criteria[indices.CriteriaIndex]
		...
		evaluations[indices.ConceptIndex].Scores[indices.CriteriaIndex] = conceptScore
		evaluations[indices.ConceptIndex].TotalWeighted += weightedScore
		
		if passes {
			evaluations[indices.ConceptIndex].PassingCount++
		}
	}
	
	// Display results per concept
	for i, evaluation := range evaluations {
		fmt.Printf("\n📊 Concept %d: %s\n", i+1, evaluation.Concept.Title)
		for _, score := range evaluation.Scores {
			fmt.Printf("   %s %s: %.3f (weighted: %.3f)\n", 
				getPassFailIcon(score.Passes), score.CriteriaName, score.Score, score.WeightedScore)
		}
	}
